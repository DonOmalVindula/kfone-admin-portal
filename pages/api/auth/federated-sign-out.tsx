...
export default async function federatedIdPLogouthandler(req: NextApiRequest, res: NextApiResponse) {
    console.log('Calling the federated-logout endpoint');
    

    try {
        const session: any = await getToken({ req });
        
        if (!session && process.env.NEXT_PUBLIC_ASGARDEO_POST_LOGOUT_REDIRECT_URI) {
        ...
        } else if (session && !session.idToken && process.env.NEXT_PUBLIC_ASGARDEO_POST_LOGOUT_REDIRECT_URI) {
            ...
            && process.env.NEXT_PUBLIC_ASGARDEO_POST_LOGOUT_REDIRECT_URI && process.env.NEXT_PUBLIC_ASGARDEO_SERVER_ORIGIN) {
        ...
        } else if (!process.env.NEXT_PUBLIC_ASGARDEO_POST_LOGOUT_REDIRECT_URI || !process.env.NEXT_PUBLIC_ASGARDEO_SERVER_ORIGIN) {
        ...
        console.log('Error while calling the /federated-logout endpoint');
